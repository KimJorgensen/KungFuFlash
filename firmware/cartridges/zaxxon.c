...
*************************************************/
static inline bool zaxxon_read_handler(uint8_t control, uint16_t addr)
{
    if (!(control & C64_ROML))
    {
        // switch upper bank
        ...
        c64_data_write(crt_banks[0][addr & 0x0fff]);
        return true;
    }
...
*************************************************/
static inline void zaxxon_write_handler(uint8_t control, uint16_t addr, uint8_t data)
{
    // No write support
}
